...
def parseAndCompare(region_type, path, id, organism, record, path_reference_file):
    if not os.path.exists(path):
        os.makedirs(path)
    
    parseFeatures(region_type, path, id, organism, record)
    assert cmp(path, path_reference_file)


def test_parseAndCompare():
    test_path = os.path.join(os.path.dirname(__file__), 'test_files', 'parsed_files')
    parameters = [(["CDS"], test_path, 'NC_000021.9', 'Homo sapiens', fetch.fetchFromID('NC_000021.9')),
                  (["CDS"], test_path, 'NC_018418.1', 'Candidatus Carsonella ruddii', fetch.fetchFromID('NC_018418.1')),
                  (["CDS"], test_path, 'NC_018416', 'Candidatus Carsonella ruddii', fetch.fetchFromID('NC_018416'))]

    for region_type, path, id, organism, record in parameters:
        parseAndCompare(region_type, path, id, organism, record, path_reference_file)
